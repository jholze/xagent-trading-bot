    ...
    *,
    ...
    for member, score in items or []:
        ...
        try:
            ...
            pass
        # Do not stamp processed here — only after the caller finishes evaluating
        # (see mark_eval_processed). Stamping at dequeue misleads staleness logic.
        jobs.append(
            EvalJob(
                symbol=symbol,
                timeframe=timeframe,
...
def mark_eval_processed(
    ...
    *,
    ...
    """Record successful evaluation completion for staleness / heartbeat logic.

    Call only after the symbol has actually been evaluated (not at dequeue time).
    """
    client = _client(config_raw)
    if not client:
        return False
    ...
    try:
        client.hset(processed_key, member, str(processed_at if processed_at is not None else time.time()))
        return True
    except Exception as e:
