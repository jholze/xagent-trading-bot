...
class LeadersBoard:
        ...
        with self._lock:
            ...
            return {
                **self._stats,
                ...
                "n_tracked_state": len(self._symbol_state),
            }
    ...
    def symbol_state_snapshot(self) -> dict[str, dict[str, Any]]:
        with self._lock:
            return {k: dict(v) for k, v in self._symbol_state.items()}
    ...
    def apply_tickers(
        ...
        *,
        ...
        max_rank_track: int = DEFAULT_SIGNAL_MAX_RANK,
        ...
        with self._lock:
            ...
            # update rank-tracking state for top-k
            top_syms = set()
            for r in leaders:
                rank = int(r.get("rank") or 999)
                sym = str(r.get("symbol") or "")
                if not sym or rank > int(max_rank_track):
                    continue
                top_syms.add(sym)
                st = self._symbol_state.get(sym) or {}
                prev_rank = st.get("rank")
                if "first_seen_top_k_at" not in st:
                    st["first_seen_top_k_at"] = now
                st["scans_in_top_k"] = int(st.get("scans_in_top_k") or 0) + 1
                st["prev_rank"] = prev_rank
                st["rank"] = rank
                st["last_seen_top_k_at"] = now
                self._symbol_state[sym] = st
                r["scans_in_top_k"] = st["scans_in_top_k"]
                r["first_seen_top_k_at"] = st["first_seen_top_k_at"]
                r["prev_rank"] = prev_rank
            # decay: drop state for symbols that left top-k for a while (keep simple: drop if not in this board top-k)
            for sym in list(self._symbol_state.keys()):
                if sym not in top_syms:
                    # keep one cycle of prev for rank_improved after drop? drop clean
                    del self._symbol_state[sym]
            self._leaders = leaders
            self._by_symbol = {r["symbol"]: r for r in leaders}
            self._last_board_at = now
            if from_rest:
    ...
    def select_signals(self, **kwargs: Any) -> list[dict[str, Any]]:
        with self._lock:
            ...
            state = {k: dict(v) for k, v in self._symbol_state.items()}
        kwargs.setdefault("prev_board", prev)
        kwargs.setdefault("symbol_state", state)
        return select_entry_signals(leaders, **kwargs)
