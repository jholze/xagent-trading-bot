...
class TradingService:
    """Unified trading facade — respects trading_mode and safety gates."""
    ...
    def execute_order(self, order: TradeOrder, timeframe: str = "4h", source: str = "manual", trust_score: float = None) -> TradeResult:
        ...
        if order.type == "BUY" and not order.usdt_amount:
            order.usdt_amount = self.max_usdt_for_order()

        result = self.adapter.execute(order, timeframe)
        if result.executed:
            log(f"{self.adapter.mode.upper()} {order.type} {order.symbol} executed", "INFO")
        return result

    def execute_buy(self, symbol: str, timeframe: str, price: float, usdt: float = None) -> TradeResult:
        order = TradeOrder(type="BUY", symbol=symbol, price=price, amount=0, usdt_amount=usdt or self.max_usdt_for_order())
