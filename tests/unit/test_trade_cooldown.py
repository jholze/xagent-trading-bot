...
class TestTradeCooldown(unittest.TestCase):
    ...
    def test_buy_blocked_after_stop_loss_sell_24h(self):
        update_position(self.symbol, self.tf, "BUY", 1.0, 100)
        update_position(self.symbol, self.tf, "SELL_STOP_FULL", 0.8, 100)
        pos = get_position(self.symbol, self.tf)
        pos["last_trade_at"] = datetime.now().isoformat()
        pos["last_trade_type"] = "SELL"
        pos["last_sell_signal"] = "SELL_STOP_FULL"

        from core.config import BotConfig
        from data_manager import get_config

        raw = dict(get_config())
        arch = raw.setdefault("architecture", {})
        ...
        self.assertFalse(decision.approved)
        self.assertEqual(decision.code, "trade_cooldown")
        self.assertIn("Stop-loss rebuy cooldown", decision.message)

    def test_buy_allowed_after_stop_loss_rebuy_24h_expires(self):
        update_position(self.symbol, self.tf, "BUY", 1.0, 100)
        update_position(self.symbol, self.tf, "SELL_STOP_FULL", 0.8, 100)
        pos = get_position(self.symbol, self.tf)
        pos["last_trade_at"] = (datetime.now() - timedelta(hours=25)).isoformat()
        pos["last_trade_type"] = "SELL"
        pos["last_sell_signal"] = "SELL_STOP_FULL"
