...
class TestTicketUsdtCap(unittest.TestCase):
    ...
    def test_dca_boost_clamped_under_neutral_cash_rich(self):
        """NEUTRAL + cash-rich DCA boost must exceed then hit max_usdt_per_trade.

        size_boost_for_regime (is_dca): 1 + (2.0-1)*0.9 = 1.9, then * cash-rich
        1.3 = 2.47 (under max_boost 2.5). Pre-cap 4000*2.47 = 9880 > 4500.
        """
        raw = {
            "max_usdt_per_trade": 4500,
            "max_position_percent": 80,
        ...
        self.assertTrue(decision.approved, decision.message)
        self.assertLessEqual(decision.order.usdt_amount, 4500.0 + 1e-6)
        # Boost > 1.125 on a 4000 base must hit the ceiling exactly (non-vacuous).
        self.assertAlmostEqual(decision.order.usdt_amount, 4500.0, places=2)
        self.assertGreater(float(decision.size_multiplier or 1.0), 1.0)
